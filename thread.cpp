...
void rp_threadReader(void *p)
{
	...
	if (readercount == 0)WaitForSingleObject(book, -1);//第一位读者申请书资源——临界
	readercount++;
...
void ReaderPriority(char *file)
{
	...
	DWORD wait_for_all;         //等待所有线程结束
								...
								//创建信号量
	rc_mutex = CreateSemaphoreW(NULL, 1, 1, L"mutex_for_readcount");//读者对count修改互斥信号量，初值为1,最大为1
	book = CreateSemaphoreW(NULL, 1, 1, NULL);//书籍互斥访问信号量，初值为1,最大值为1
...
void WriterPriority(char *file) {
	...
	DWORD wait_for_all;         //等待所有线程结束
								...
								//创建信号量
	rc2_mutex = CreateSemaphoreW(NULL, 1, 1,L"mutex_for_readercount");//读者对count修改互斥信号量，初值为1,最大为1
	wc_mutex = CreateSemaphoreW(NULL, 1, 1, L"mutex_for_writercount");//写者对count修改互斥信号量，初值为1,最大为1
	wrt = CreateSemaphoreW(NULL, 1, 1, NULL);//
	mutex = CreateSemaphoreW(NULL, 1, 1, NULL);//
	book = CreateSemaphoreW(NULL, 1, 1, NULL);//书籍互斥访问信号量，初值为1,最大值为1
...
int main()
{
	...
	while (true)
	{
		...
		if (choice == '1')
			...
			ReaderPriority(const_cast<char *>("D:\\cpp\\thread.txt"));
		//选择2，写者优先1
		else if (choice == '2')
			WriterPriority(const_cast<char *>("D:\\cpp\\thread.txt"));
		//选择3，退出
		else
			return 0;
		//结束
